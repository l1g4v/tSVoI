...
use std::sync::{Arc, Mutex, Condvar, atomic::AtomicI8};
...
impl AudioCapture {
    ...
    pub fn new(device_config: DeviceConfig, encoder_bitrate: i32, active_threshold: i8) -> Self{
        ...
        let encoder = Arc::new(Mutex::new(Encoder::new(device_config.sample_rate(), encoder_channels, Application::Voip).unwrap()));
        ...
        capture_device.set_data_callback(move |_, _, input|{
            ...
            if rms > threshold_clone.load(std::sync::atomic::Ordering::Relaxed){
                let encoded = Bytes::from(
                    encoder_clone.lock().unwrap().encode_vec(input_samples, num_samples).unwrap()
                );
                capture_tx_clone.send(encoded).unwrap();
            }
        });
        AudioCapture { capture_device, capture_tx, capture_rx, intensity_tx, intensity_rx, threshold, encoder}
    }
    ...
    pub fn change_device(&mut self, device_id: DeviceId, channels: u32, sample_rate: u32,){
        ...
        capture_device.set_data_callback(move |_, _, input|{
            ...
            if rms > threshold.load(std::sync::atomic::Ordering::Relaxed){
                ...
                let _ = encoder_clone.lock().unwrap().encode(input_samples, encoded.as_mut()).unwrap();
